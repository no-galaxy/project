...
inputs[1].onblur = function() {
    if (this.value != '') {
        if (!regEmail.test(this.value)) {
            ...
            labels[1].innerHTML = '请输入正确的邮箱格式';
        } else {
            labels[1].className = 'right';
            labels[1].innerHTML = '邮箱格式输入正确';
        }
    } else {
        labels[1].className = '';
        labels[1].innerHTML = 'Email';
    }
};
...
btn.onclick = function() {
    ...
    ajax ( {
        ...
        url: 'http://175.178.51.126:8091/smallA/deleteUser',
        data: {
        ...
        success: function(result, xhr) {
            ...
            span.innerHTML = '注销成功，重新注册可返回注册页面';
        } else {
